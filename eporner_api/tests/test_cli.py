...
from eporner_api.api import create_parser, run_main
...
class TestCLI(unittest.IsolatedAsyncioTestCase):
    ...
    def test_parser_custom_args(self):
        ...
        args = parser.parse_args([
            "--download", "https://eporner.com/video-12345",
            "--output", "/tmp/out",
            "--quality", "worst",
            "--no-title", "True"
        ])
        self.assertEqual(args.download, "https://eporner.com/video-12345")
        self.assertEqual(args.output, "/tmp/out")
    ...
    async def test_run_main_download(self):
        ...
        with patch("eporner_api.api.Client", return_value=mock_client):
            await run_main(["--download", "https://eporner.com/video-12345", "--output", "/tmp/out"])

        mock_client.get_video.assert_awaited_once_with("https://eporner.com/video-12345", load_html=True)
    ...
    async def test_run_main_file(self):
        ...
        with tempfile.NamedTemporaryFile("w+", delete=False) as f:
            f.write("https://eporner.com/v1\nhttps://eporner.com/v2\n")
            f.flush()
            temp_name = f.name
